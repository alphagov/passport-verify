...
export class PassportVerifyStrategy extends Strategy {
  ...
  constructor (private client: VerifyServiceProviderClient,
               ...
               private setRequestId: (requestId: string, request: express.Request) => any,
               private getRequestId: (request: express.Request) => string) {
    super()
  }
  ...
  private async _renderAuthnRequest (request: express.Request): Promise<express.Response> {
    ...
    if (authnRequestResponse.status === 200) {
      ...
      this.setRequestId(authnRequestResponseBody.requestId, request)
 ...
 * @param setRequestId A callback that will be invoked to store the requestId that has been generated by
 * the verify service provider. Your callback should store the request Id in a secure manner so that it
 * can be matched against the corresponding SAML response.
 * @param getRequestId A callback that will be invoked to retrieve the requestId that was securely stored
...
export function createStrategy (
  ...
  setRequestId: (requestId: string, request: express.Request) => void,
  getRequestId: (request: express.Request) => string
) {
  const client = new VerifyServiceProviderClient(verifyServiceProviderHost, logger || { info: () => undefined })
  return new PassportVerifyStrategy(client, createUser, verifyUser, setRequestId, getRequestId)
}
